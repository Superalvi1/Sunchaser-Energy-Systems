...
export type QuoteTemplateWorkspaceProps = {
  ...
  onSaveGlobalSettings?: () => Promise<boolean>;
};
...
export default function QuoteTemplateWorkspace(props: QuoteTemplateWorkspaceProps) {
  ...
  const handleDownloadTestPdf = async (scope: "page" | "full") => {
    try {
      if (scope === "full" && onSaveGlobalSettings) {
        setPdfActionStatus("Saving…");
        const globalOk = await onSaveGlobalSettings();
        if (!globalOk) return;
      }
  ...
  const handlePrintPreview = async () => {
    if (selectedTemplatePageId === TEMPLATE_NAV_GLOBAL || selectedTemplatePageId === TEMPLATE_NAV_BOQ) {
      window.open(
        `${API_BASE_URL}/api/export/pdf/template-preview/${encodeURIComponent(selectedTemplateId)}?scope=full`,
        "_blank",
        "noopener,noreferrer"
      );
      return;
      ...
      <header className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 bg-slate-900 px-4 py-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[9px] text-slate-600 truncate">Print Preview may vary. Download PDF uses server A4 engine.</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {selectedTemplatePageId === TEMPLATE_NAV_GLOBAL ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <button type="button" onClick={() => handleDownloadTestPdf("full")} title="Downloads final A4 PDF generated by server" className="bg-violet-600 text-white font-bold px-5 py-2.5 rounded-lg">
