...
export type QuoteTemplateStudioProps = {
  ...
  onSavePageAsync?: (pageId: string) => Promise<boolean>;
...
export default function QuoteTemplateStudio({
  ...
  const printPreview = () => {
    if (activeNav === TEMPLATE_NAV_BOQ || activeNav === TEMPLATE_NAV_GLOBAL) {
      window.open(`${API_BASE_URL}/api/export/pdf/template-preview/${encodeURIComponent(selectedTemplateId)}?scope=full`, "_blank");
      return;
    }
    openPagePreview();
  };
  ...
  const handleDownloadTestPdf = async (scope: "page" | "full") => {
    try {
      if (scope === "page" && activePage && onSavePageAsync) {
        setPdfActionStatus("Saving…");
        const ok = await onSavePageAsync(activePage.id);
        if (!ok) return;
      }
      ...
      <div className="flex items-center justify-between gap-3 border-b border-slate-800 px-4 py-3 bg-slate-950 shrink-0">
        ...
        {activePage && pageState && (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={printPreview} title="Browser print may vary. Use Download PDF for final client sharing." className="text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-slate-800 text-slate-200 hover:bg-slate-700 flex items-center gap-1">
            ...
            <button
              ...
              onClick={() => void handleDownloadTestPdf("page")}
              className="text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-violet-700 text-white hover:bg-violet-600 disabled:opacity-50 flex items-center gap-1"
            >
