 ...
 * Mounted before global JWT auth so normal browser downloads can follow links,
 * but every request must carry the HMAC generated by the backend.
 */
app.get("/api/storage/object/:namespace/:encodedKey", async (req, res) => {
  try {
    const verified = verifyRailwayObjectProxySignature(
      String(req.params.namespace || ""),
      String(req.params.encodedKey || ""),
      String(req.query.sig || ""),
    );
    if (!verified.ok) return res.status(403).json({ error: "Invalid object signature." });
    ...
    console.error("[Railway object storage]", err?.message || err);
    return res.status(503).json({ error: "Object storage unavailable." });
  }
});
