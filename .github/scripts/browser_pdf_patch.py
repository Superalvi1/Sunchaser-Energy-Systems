...
# Replace native filesystem/share imports with Browser-only handoff.
...
# Remove blob-to-base64 helper if present.
s = re.sub(r'async function blobToBase64\(blob: Blob\): Promise<string> \{.*?\n\}\n\n', '', s, count=1, flags=re.S)

# Replace any native block inside triggerBlobDownload so browser/web remains unchanged.
...
# Helper to request a tiny JSON staging response then open outside the WebView.
...
# Quick quotation: stage server-side; browser web remains blob download.
...
# Saved manual quote: POST to authenticated staging endpoint, then Browser.open one-time URL.
...
# AutoSizer saved quote uses same staging endpoint.
...
# Server staging: random one-time token, short TTL, public GET only by unguessable token.
...
# Quick quotation staging branch.
...
# Saved quote staging endpoint. Reuses existing compile/export logic by internally resolving lead/quote/state.
...
# Regression test: native path must only stage small JSON + Browser.open.
