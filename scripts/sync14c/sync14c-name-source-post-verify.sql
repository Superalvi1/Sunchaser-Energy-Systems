-- =============================================================================
-- SYNC-14C-A — post-migration verification (READ-ONLY)
-- =============================================================================
...
begin
  ...
  if def is null then
    ...
    if def not ilike '%whatsapp_verified%' then
      stop_reasons := array_append(stop_reasons, 'constraint missing whatsapp_verified');
    end if;
    if def not ilike '%whatsapp_legacy%' then
      stop_reasons := array_append(stop_reasons, 'constraint missing whatsapp_legacy');
    end if;
    if def not ilike '%manual%' or def not ilike '%whatsapp_saved%' then
      stop_reasons := array_append(stop_reasons, 'constraint missing preserved legacy values');
    end if;
    if not exists (
      select 1
      from pg_constraint con
  ...
  if exists (
    ...
    where conname = 'whatsapp_contacts_name_source_check_v14c'
